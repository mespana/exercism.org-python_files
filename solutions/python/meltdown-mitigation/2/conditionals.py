...
def is_criticality_balanced(temperature:float, neutrons_emitted:float) -> bool:
    ...
    emissions = float(temperature * neutrons_emitted)
    
    if temperature < 800 and neutrons_emitted > 500 and emissions < 500000:
        return True
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    threshold_10_minus = float(threshold * .10)
    threshold_10_plus = float(threshold * 1.10)
    
    if product < (threshold * .90):
        return 'LOW'
    elif threshold_10_minus < product < threshold_10_plus:
        return 'NORMAL'
